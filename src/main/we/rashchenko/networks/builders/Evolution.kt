 ...
 * That neurons are notified by update function (calling update considered as warning).
 * If some [Neuron] warned [warningsBeforeKill] times (in total, not only consequently) it is replaced with other random
 *  [Neuron] by calling remove and add functions of the wrapped builder.
 * So, as you can see that class implements only bad neurons killing,
 *  but sampling of new successful neurons is already managed by base builder.
 */
class Evolution(
